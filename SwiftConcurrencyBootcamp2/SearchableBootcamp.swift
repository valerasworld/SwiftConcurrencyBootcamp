...
// Implement the Searchable, Search Suggestions, and Search Scopes modifiers in SwiftUI
// to enhance the search functionality of your iOS apps out of the box!
            ...
            VStack(spacing: 20) {
                ForEach(viewModel.isSearching ? viewModel.filteredRestaurants : viewModel.allRestaurants) { restaurant in
                    NavigationLink(value: restaurant) {
                        restaurantRow(restaurnat: restaurant)
                    }
                }
            }
            .padding()
...
//        .navigationBarTitleDisplayMode(.inline)
    ...
    private func restaurantRow(restaurnat: Restaurant) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(restaurnat.title)
                .font(.headline)
            Text(restaurnat.cuisine.rawValue.capitalized)
                .font(.caption)
        }
